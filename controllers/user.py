...
async def post(data: UserSchemaIn, session: Session = Depends(get_db_session)):
    ...
    data.update({
        'password': AuthService.hash_password(data.get('password')),
    })
    return UserMapper.create(session=session, data=data)


@user_router.put("", response_model=UserSchemaOut, dependencies=[Security(IsAuthenticated())])
